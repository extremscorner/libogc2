...
static struct netif *w6100_netif;
...
static bool W6100_ReadReg(s32 chan, W6100Reg addr, u8 *data)
{
	return W6100_ReadCmd(chan, W6100_OM(1) | addr, data, 1);
}

static bool W6100_WriteReg(s32 chan, W6100Reg addr, u8 data)
{
	return W6100_WriteCmd(chan, W6100_OM(1) | addr, &data, 1);
}

static bool W6100_ReadReg16(s32 chan, W6100Reg16 addr, u16 *data)
{
	u16 tmp;

	do {
		if (!W6100_ReadCmd(chan, W6100_OM(2) | addr, data, 2) ||
			!W6100_ReadCmd(chan, W6100_OM(2) | addr, &tmp, 2))
			return false;
	} while (*data != tmp);
...
static bool W6100_WriteReg16(s32 chan, W6100Reg16 addr, u16 data)
{
	return W6100_WriteCmd(chan, W6100_OM(2) | addr, &data, 2);
}

#if 0
static bool W6100_ReadReg32(s32 chan, W6100Reg32 addr, u32 *data)
{
	u32 tmp;

	do {
		if (!W6100_ReadCmd(chan, W6100_OM(3) | addr, data, 4) ||
			!W6100_ReadCmd(chan, W6100_OM(3) | addr, &tmp, 4))
			return false;
	} while (*data != tmp);
...
static bool W6100_WriteReg32(s32 chan, W6100Reg32 addr, u32 data)
{
	return W6100_WriteCmd(chan, W6100_OM(3) | addr, &data, 4);
}
#endif

static bool W6100_Reset(s32 chan)
{
	if (!W6100_WriteReg(chan, W6100_CHPLCKR, W6100_CHPLCKR_UNLOCK) ||
		!W6100_WriteReg(chan, W6100_SYCR0, 0) ||
		!W6100_WriteReg(chan, W6100_CHPLCKR, W6100_CHPLCKR_LOCK))
		return false;

	return true;
}

static bool W6100_GetLinkState(s32 chan)
{
	u8 physr;

	if (!W6100_ReadReg(chan, W6100_PHYSR, &physr))
		return false;

	return !!(physr & W6100_PHYSR_LNK);
}

static void W6100_GetMACAddr(s32 chan, u8 macaddr[6])
{
	union {
		u32 cid[4];
		u8 data[18 + 1];
...
{
	struct w6100if *w6100if = w6100_netif->state;
	chan = w6100if->chan;
	dev = w6100if->dev;
	...
	u8 ir, sir;
	W6100_WriteReg(chan, W6100_SIMR, 0);
	W6100_ReadReg(chan, W6100_SIR, &sir);

	if (sir & W6100_SIR_S(0)) {
		W6100_ReadReg(chan, W6100_S0_IR, &ir);
	...
	W6100_WriteReg(chan, W6100_SIMR, W6100_SIMR_S(0));

	if (W6100_GetLinkState(chan))
		w6100_netif->flags |= NETIF_FLAG_LINK_UP;
	else
		w6100_netif->flags &= ~NETIF_FLAG_LINK_UP;
...
{
	struct w6100if *w6100if = w6100_netif->state;
	LWP_ThreadBroadcast(w6100if->unlockQueue);
	return TRUE;
}

static bool W6100_Init(s32 chan, s32 dev, struct w6100if *w6100if)
{
	bool err = false;
	u8 sr;
	u16 cidr, ver;
	...
	err |= !W6100_WriteReg(chan, W6100_SHAR0, w6100if->ethaddr->addr[0]);
	err |= !W6100_WriteReg(chan, W6100_SHAR1, w6100if->ethaddr->addr[1]);
	err |= !W6100_WriteReg(chan, W6100_SHAR2, w6100if->ethaddr->addr[2]);
	err |= !W6100_WriteReg(chan, W6100_SHAR3, w6100if->ethaddr->addr[3]);
	err |= !W6100_WriteReg(chan, W6100_SHAR4, w6100if->ethaddr->addr[4]);
	...
	for (int n = 1; n < 8; n++) {
		...
		err |= !W6100_WriteReg(chan, W6100_REG_S(n, Sn_RX_BSR), 0);
	}

	err |= !W6100_WriteReg(chan, W6100_S0_MR, W6100_Sn_MR_MF | W6100_Sn_MR_MACRAW);
	err |= !W6100_WriteReg(chan, W6100_S0_CR, W6100_Sn_CR_OPEN);
	err |= !W6100_ReadReg(chan, W6100_S0_SR, &sr);

	if (err || sr != W6100_Sn_SR_MACRAW) {
		EXI_Unlock(chan);
		return false;
	}

	err |= !W6100_WriteReg(chan, W6100_S0_IMR, W6100_Sn_IMR_SENDOK | W6100_Sn_IMR_RECV);
	err |= !W6100_WriteReg(chan, W6100_SIMR, W6100_SIMR_S(0));
...
static err_t w6100_output(struct netif *netif, struct pbuf *p)
{
	struct w6100if *w6100if = netif->state;
	s32 chan = w6100if->chan;
	s32 dev = w6100if->dev;
...
static err_t w6100if_output(struct netif *netif, struct pbuf *p, struct ip_addr *ipaddr)
{
...
{
	struct w6100if *w6100if = mem_malloc(sizeof(struct w6100if));

	if (w6100if == NULL) {
		LWIP_DEBUGF(NETIF_DEBUG, ("w6100if_init: out of memory\n"));
		return ERR_MEM;
	}

	netif->state = w6100if;
	netif->output = w6100if_output;
	netif->linkoutput = w6100_output;
	...
	if (W6100_Init(EXI_CHANNEL_0, EXI_DEVICE_2, w6100if)) {
		netif->name[0] = 'w';
		netif->name[1] = '1';
		return ERR_OK;
	}

	if (W6100_Init(EXI_CHANNEL_2, EXI_DEVICE_0, w6100if)) {
		netif->name[0] = 'w';
		netif->name[1] = '2';
		return ERR_OK;
	}
	...
	for (s32 chan = EXI_CHANNEL_0; chan < EXI_CHANNEL_2; chan++) {
		if (W6100_Init(chan, EXI_DEVICE_0, w6100if)) {
			netif->name[0] = 'w';
			netif->name[1] = 'A' + chan;
			return ERR_OK;
		}
