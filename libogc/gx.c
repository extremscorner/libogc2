...
GXFifoObj* GX_Init(void *base,u32 size)
{
	...
	memset(&__gxregs,0,sizeof(struct __gx_regdef));
...
void GX_BeginDispList(void *list,u32 size)
{
	...
	if(__gx->saveDLctx)
		memcpy(&_gx_saved_data,&__gxregs,sizeof(struct __gx_regdef));
...
u32 GX_EndDispList(void)
{
	...
	if(__gx->saveDLctx) {
		_CPU_ISR_Disable(level);
		memcpy(&__gxregs,&_gx_saved_data,sizeof(struct __gx_regdef));
		_CPU_ISR_Restore(level);
	}
...
void GX_AdjustForOverscan(const GXRModeObj *rmin,GXRModeObj *rmout,u16 hor,u16 ver)
{
	...
	if((rmin->xfbMode&0x3)==VI_XFBMODE_SF && (rmin->viTVMode&0x3)==VI_INTERLACE) rmout->xfbHeight = rmin->xfbHeight-ver;
	else if((rmin->xfbMode&0x3)!=VI_XFBMODE_SF && (rmin->viTVMode&0x3)!=VI_INTERLACE) rmout->xfbHeight = rmin->xfbHeight-(ver<<2);
